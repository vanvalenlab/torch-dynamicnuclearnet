...
@click.option(
    '--data-path', 
    default=Path.home() / ".deepcell/dnn/test.zarr" , 
    help="""Path to the Zarr file containing the test data for evaluation.
            If unset, defaults to DeepCell location (`~/.deepcell/dnn/test.zarr)"""
        )
...
def main(device: str,
    ...
    for i in tqdm.tqdm(range(X_test.shape[0])):
        ...
        preds = model.predict(X_test[i:i+1], image_mpp=mpps[i], postprocess_kwargs=default_kwargs, batch_size=8)
        y_true = y_test[i:i+1]
